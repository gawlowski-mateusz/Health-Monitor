...
class WalkingList(MethodView):
    ...
    def get(self):
        ...
        training_id = activity.training_id
        return WalkingModel.query.filter_by(training_id=training_id, date=datetime.today().strftime('%Y-%m-%d')).all()
    ...
    def post(self, walking_data):
        ...
        # Create new walking session for current day
        activity = ActivityModel.query.filter_by(user_id=user_id, date=datetime.today().strftime('%Y-%m-%d')).first()
        walking = WalkingModel(average_pulse=walking_data["average_pulse"],
                               duration=walking_data["duration"],
                               date=datetime.today().strftime('%Y-%m-%d'),
                               training_id=activity.training_id)
        try:
