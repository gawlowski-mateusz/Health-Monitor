...
class LoginUnitTests {
    ...
    fun setup() {
        ...
        every { anyConstructed<JSONObject>().getString(any()) } returns "test_token"
        every { anyConstructed<JSONObject>().toString() } returns "{\"email\":\"test@example.com\",\"password\":\"password123\"}"
    }

    @Test
    fun `test successful login`() = runBlocking {
        // Prepare mock response
        val successResponse = """{"access_token": "test_token"}"""
        ...
        // Execute login request with mock connection
    ...
    fun `test invalid credentials`() = runBlocking {
        ...
        val errorResponse = """{"message": "Invalid credentials"}"""
        ...
        with(mockConnection) {
            ...
            every { doOutput = any() } just runs
            every { outputStream } returns ByteArrayOutputStream()
            every { responseCode } returns 401
        ...
        every { anyConstructed<JSONObject>().getString("message") } returns "Invalid credentials"
    ...
    @Test
    fun `test network error`() = runBlocking {
    ...
    @Test
    fun `test SSL error`() = runBlocking {
        ...
        // Verify error message
        assertEquals("Error: SSL certificate verification failed", result)
    }
}