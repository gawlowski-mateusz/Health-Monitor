...
class RegisterUnitTests {
    ...
    fun setup() {
        ...
        every { anyConstructed<JSONObject>().toString() } returns "{\"email\":\"test@example.com\"}"
    }

    @Test
    fun `test successful registration with all fields`() = runBlocking {
        // Prepare mock response
        val successResponse = """{"message": "User created successfully"}"""
        ...
        with(mockConnection) {
            ...
            every { responseCode } returns 201
            every { inputStream } returns ByteArrayInputStream(successResponse.toByteArray())
            every { disconnect() } just runs
            every { sslSocketFactory = any() } just runs
            every { hostnameVerifier = any() } just runs
        }

        // Execute registration request with mock connection
        val result = makeRegisterRequest(
            name = "Test User",
    ...
    @Test
    fun `test registration with missing optional fields`() = runBlocking {
        // Prepare mock response
        val successResponse = """{"message": "User created successfully"}"""
    ...
    @Test
    fun `test registration with email already taken`() = runBlocking {
        // Prepare mock error response
        val errorResponse = """{"message": "Email already registered"}"""
        ...
        every { anyConstructed<JSONObject>().getString("message") } returns "Email already registered"
    ...
    @Test
    fun `test registration with network error`() = runBlocking {
        ...
        // Verify error message
        assertEquals("Error: No internet connection", result)
    }

    @Test
    fun `test registration with invalid data`() = runBlocking {
        // Prepare mock error response
        val errorResponse = """{"message": "Invalid input data"}"""
        ...
        // Mock JSONObject for error response
        every { anyConstructed<JSONObject>().getString("message") } returns "Invalid input data"
        ...
        val result = makeRegisterRequest(
            ...
            birthDate = null,
            gender = "Invalid",
            weight = "abc",  // Invalid weight format
