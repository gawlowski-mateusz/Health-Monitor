package com.mateusz.frontend

import android.content.Context
import android.content.SharedPreferences
import android.content.res.Resources
import io.mockk.*
import kotlinx.coroutines.runBlocking
import org.junit.Before
import org.junit.Test
import org.junit.Assert.*
import java.io.ByteArrayInputStream
import java.io.ByteArrayOutputStream
import javax.net.ssl.HttpsURLConnection
import org.json.JSONObject
import java.net.HttpURLConnection

class WalkingSessionUnitTests {
    private lateinit var mockContext: Context
    private lateinit var mockSharedPrefs: SharedPreferences
    private lateinit var mockResources: Resources
    private lateinit var mockConnection: HttpsURLConnection

    @Before
    fun setup() {
        mockContext = mockk(relaxed = true)
        mockSharedPrefs = mockk(relaxed = true)
        mockResources = mockk(relaxed = true)
        mockConnection = mockk(relaxed = true)

        // Mock SharedPreferences
        every { mockContext.getSharedPreferences("auth", Context.MODE_PRIVATE) } returns mockSharedPrefs
        every { mockSharedPrefs.getString("access_token", any()) } returns "test_token"

        // Mock Resources with certificate input stream
        val dummyCertData = """
            -----BEGIN CERTIFICATE-----
            
            -----END CERTIFICATE-----
        """.trimIndent().toByteArray()

        every { mockContext.resources } returns mockResources
        every { mockResources.openRawResource(any()) } returns ByteArrayInputStream(dummyCertData)

        // Mock NetworkConfig
        mockkObject(NetworkConfig)
        every { NetworkConfig.getBaseUrl() } returns "https://10.0.2.2:443"
        every { NetworkConfig.isEmulator() } returns true
    }

    @Test
    fun `test successful walking session addition`() = runBlocking {
        // Setup connection expectations
        with(mockConnection) {
            every { requestMethod = any() } just runs
            every { setRequestProperty(any(), any()) } just runs
            every { doOutput = any() } just runs
            every { outputStream } returns ByteArrayOutputStream()
            every { responseCode } returns HttpURLConnection.HTTP_CREATED
            every { disconnect() } just runs
            every { sslSocketFactory = any() } just runs
            every { hostnameVerifier = any() } just runs
        }

        // Mock JSONObject for request
        mockkConstructor(JSONObject::class)
        every { anyConstructed<JSONObject>().put("average_pulse", 120) } returns mockk()
        every { anyConstructed<JSONObject>().put("duration", 45) } returns mockk()
        every { anyConstructed<JSONObject>().toString() } returns """{"average_pulse":120,"duration":45}"""

        // Execute request
        val result = makeAddNewWalkingSessionRequest(
            duration = 45,
            averagePulse = 120,
            password = null,
            context = mockContext,
            testConnection = mockConnection
        )

        // Verify success
        assertTrue(result is WalkingSessionResult.Success)
    }

    @Test
    fun `test network error walking session addition`() = runBlocking {
        // Setup connection to throw network error
        every { mockConnection.setRequestProperty(any(), any()) } throws java.net.UnknownHostException()

        // Execute request
        val result = makeAddNewWalkingSessionRequest(
            duration = 45,
            averagePulse = 120,
            password = null,
            context = mockContext,
            testConnection = mockConnection
        )

        // Verify error
        assertTrue(result is WalkingSessionResult.Error)
        assertEquals("No internet connection", (result as WalkingSessionResult.Error).message)
    }

    @Test
    fun `test SSL error walking session addition`() = runBlocking {
        // Setup connection to throw SSL error
        every { mockConnection.setRequestProperty(any(), any()) } throws javax.net.ssl.SSLHandshakeException("Invalid certificate")

        // Execute request
        val result = makeAddNewWalkingSessionRequest(
            duration = 45,
            averagePulse = 120,
            password = null,
            context = mockContext,
            testConnection = mockConnection
        )

        // Verify error
        assertTrue(result is WalkingSessionResult.Error)
        assertEquals("SSL certificate verification failed", (result as WalkingSessionResult.Error).message)
    }

    @Test
    fun `test invalid number format`() = runBlocking {
        // Setup connection to throw number format error
        every { mockConnection.setRequestProperty(any(), any()) } throws NumberFormatException("Invalid number")

        // Execute request
        val result = makeAddNewWalkingSessionRequest(
            duration = -1,
            averagePulse = 120,
            password = null,
            context = mockContext,
            testConnection = mockConnection
        )

        // Verify error
        assertTrue(result is WalkingSessionResult.Error)
        assertEquals("Invalid number format for duration or pulse", (result as WalkingSessionResult.Error).message)
    }

    @Test
    fun `test invalid walking session data`() = runBlocking {
        // Prepare mock error response
        val errorResponse = """{"message": "Invalid pulse rate"}"""

        with(mockConnection) {
            every { requestMethod = any() } just runs
            every { setRequestProperty(any(), any()) } just runs
            every { doOutput = any() } just runs
            every { outputStream } returns ByteArrayOutputStream()
            every { responseCode } returns HttpURLConnection.HTTP_BAD_REQUEST
            every { errorStream } returns ByteArrayInputStream(errorResponse.toByteArray())
            every { disconnect() } just runs
            every { sslSocketFactory = any() } just runs
            every { hostnameVerifier = any() } just runs
        }

        // Mock JSONObject for both request and error response
        mockkConstructor(JSONObject::class)
        // Mock for request JSONObject
        every { anyConstructed<JSONObject>().put("average_pulse", 250) } returns mockk()
        every { anyConstructed<JSONObject>().put("duration", 45) } returns mockk()
        every { anyConstructed<JSONObject>().toString() } returns """{"average_pulse":250,"duration":45}"""
        // Mock for error response
        every { anyConstructed<JSONObject>().getString("message") } returns "Invalid pulse rate"

        // Execute request
        val result = makeAddNewWalkingSessionRequest(
            duration = 45,
            averagePulse = 250,
            password = null,
            context = mockContext,
            testConnection = mockConnection
        )

        // Verify error
        assertTrue(result is WalkingSessionResult.Error)
        assertEquals("Invalid pulse rate", (result as WalkingSessionResult.Error).message)
    }

    @Test
    fun `test unauthorized walking session addition`() = runBlocking {
        // Setup connection for unauthorized response
        with(mockConnection) {
            every { requestMethod = any() } just runs
            every { setRequestProperty(any(), any()) } just runs
            every { doOutput = any() } just runs
            every { outputStream } returns ByteArrayOutputStream()
            every { responseCode } returns HttpURLConnection.HTTP_UNAUTHORIZED
            every { disconnect() } just runs
            every { sslSocketFactory = any() } just runs
            every { hostnameVerifier = any() } just runs
        }

        // Mock JSONObject for request
        mockkConstructor(JSONObject::class)
        every { anyConstructed<JSONObject>().put("average_pulse", 120) } returns mockk()
        every { anyConstructed<JSONObject>().put("duration", 45) } returns mockk()
        every { anyConstructed<JSONObject>().toString() } returns """{"average_pulse":120,"duration":45}"""

        // Execute request
        val result = makeAddNewWalkingSessionRequest(
            duration = 45,
            averagePulse = 120,
            password = null,
            context = mockContext,
            testConnection = mockConnection
        )

        // Verify error
        assertTrue(result is WalkingSessionResult.Error)
        assertEquals("Session expired. Please login again", (result as WalkingSessionResult.Error).message)
    }

    @Test
    fun `test server error walking session addition`() = runBlocking {
        // Setup connection for server error response
        with(mockConnection) {
            every { requestMethod = any() } just runs
            every { setRequestProperty(any(), any()) } just runs
            every { doOutput = any() } just runs
            every { outputStream } returns ByteArrayOutputStream()
            every { responseCode } returns HttpURLConnection.HTTP_INTERNAL_ERROR
            every { disconnect() } just runs
            every { sslSocketFactory = any() } just runs
            every { hostnameVerifier = any() } just runs
        }

        // Mock JSONObject for request
        mockkConstructor(JSONObject::class)
        every { anyConstructed<JSONObject>().put("average_pulse", 120) } returns mockk()
        every { anyConstructed<JSONObject>().put("duration", 45) } returns mockk()
        every { anyConstructed<JSONObject>().toString() } returns """{"average_pulse":120,"duration":45}"""

        // Execute request
        val result = makeAddNewWalkingSessionRequest(
            duration = 45,
            averagePulse = 120,
            password = null,
            context = mockContext,
            testConnection = mockConnection
        )

        // Verify error
        assertTrue(result is WalkingSessionResult.Error)
        assertEquals("Server error. Please try again later", (result as WalkingSessionResult.Error).message)
    }
}