...
class StepsGoalUpdateUnitTests {
    ...
    fun setup() {
        ...
        every { anyConstructed<JSONObject>().toString() } returns "{\"goal\":10000}"
    }

    @Test
    fun `test successful steps goal update`() = runBlocking {
        ...
        val result = makeEditStepsRequest(
            goal = 10000,
            context = mockContext,
    ...
    @Test
    fun `test update without authentication token`() = runBlocking {
    ...
    @Test
    fun `test update with invalid goal value`() = runBlocking {
        // Setup connection to return bad request
        val errorResponse = """{"message": "Goal must be a positive number"}"""
        ...
        every { anyConstructed<JSONObject>().getString("message") } returns "Goal must be a positive number"
        ...
        // Verify error
        assertTrue(result is StepsUpdateResult.Error)
        assertEquals("Goal must be a positive number", (result as StepsUpdateResult.Error).message)
    }
