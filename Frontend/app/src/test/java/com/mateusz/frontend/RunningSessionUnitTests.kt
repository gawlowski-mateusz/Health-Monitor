package com.mateusz.frontend

import android.content.Context
import android.content.SharedPreferences
import android.content.res.Resources
import io.mockk.*
import kotlinx.coroutines.runBlocking
import org.junit.Before
import org.junit.Test
import org.junit.Assert.*
import java.io.ByteArrayInputStream
import java.io.ByteArrayOutputStream
import javax.net.ssl.HttpsURLConnection
import org.json.JSONObject
import java.net.HttpURLConnection

class RunningSessionUnitTests {
    private lateinit var mockContext: Context
    private lateinit var mockSharedPrefs: SharedPreferences
    private lateinit var mockResources: Resources
    private lateinit var mockConnection: HttpsURLConnection

    @Before
    fun setup() {
        mockContext = mockk(relaxed = true)
        mockSharedPrefs = mockk(relaxed = true)
        mockResources = mockk(relaxed = true)
        mockConnection = mockk(relaxed = true)

        // Mock SharedPreferences
        every { mockContext.getSharedPreferences("auth", Context.MODE_PRIVATE) } returns mockSharedPrefs
        every { mockSharedPrefs.getString("access_token", any()) } returns "test_token"

        // Mock Resources with certificate input stream
        val dummyCertData = """
            -----BEGIN CERTIFICATE-----
            
            -----END CERTIFICATE-----
        """.trimIndent().toByteArray()

        every { mockContext.resources } returns mockResources
        every { mockResources.openRawResource(any()) } returns ByteArrayInputStream(dummyCertData)

        // Mock NetworkConfig
        mockkObject(NetworkConfig)
        every { NetworkConfig.getBaseUrl() } returns "https://10.0.2.2:443"
        every { NetworkConfig.isEmulator() } returns true
    }

    @Test
    fun `test successful running session addition`() = runBlocking {
        // Setup connection expectations
        with(mockConnection) {
            every { requestMethod = any() } just runs
            every { setRequestProperty(any(), any()) } just runs
            every { doOutput = any() } just runs
            every { outputStream } returns ByteArrayOutputStream()
            every { responseCode } returns HttpURLConnection.HTTP_CREATED
            every { disconnect() } just runs
            every { sslSocketFactory = any() } just runs
            every { hostnameVerifier = any() } just runs
        }

        // Mock JSONObject for success case
        mockkConstructor(JSONObject::class)
        every { anyConstructed<JSONObject>().put(any(), any<Int>()) } returns mockk()
        every { anyConstructed<JSONObject>().toString() } returns "{\"duration\":30,\"average_pulse\":140}"

        // Execute request
        val result = makeAddNewRunningSessionRequest(
            duration = 30,
            averagePulse = 140,
            password = null,
            context = mockContext,
            testConnection = mockConnection
        )

        // Verify success
        assertTrue(result is RunningSessionResult.Success)
    }

    @Test
    fun `test network error running session addition`() = runBlocking {
        // Setup connection to throw network error
        every { mockConnection.setRequestProperty(any(), any()) } throws java.net.UnknownHostException()

        // Execute request
        val result = makeAddNewRunningSessionRequest(
            duration = 30,
            averagePulse = 140,
            password = null,
            context = mockContext,
            testConnection = mockConnection
        )

        // Verify error
        assertTrue(result is RunningSessionResult.Error)
        assertEquals("No internet connection", (result as RunningSessionResult.Error).message)
    }

    @Test
    fun `test SSL error running session addition`() = runBlocking {
        // Setup connection to throw SSL error
        every { mockConnection.setRequestProperty(any(), any()) } throws javax.net.ssl.SSLHandshakeException("Invalid certificate")

        // Execute request
        val result = makeAddNewRunningSessionRequest(
            duration = 30,
            averagePulse = 140,
            password = null,
            context = mockContext,
            testConnection = mockConnection
        )

        // Verify error
        assertTrue(result is RunningSessionResult.Error)
        assertEquals("SSL certificate verification failed", (result as RunningSessionResult.Error).message)
    }

    @Test
    fun `test invalid number format`() = runBlocking {
        // Setup connection to throw number format error
        every { mockConnection.setRequestProperty(any(), any()) } throws NumberFormatException("Invalid number")

        // Execute request
        val result = makeAddNewRunningSessionRequest(
            duration = -1,
            averagePulse = 140,
            password = null,
            context = mockContext,
            testConnection = mockConnection
        )

        // Verify error
        assertTrue(result is RunningSessionResult.Error)
        assertEquals("Invalid number format for duration or pulse", (result as RunningSessionResult.Error).message)
    }

    @Test
    fun `test invalid running session data`() = runBlocking {
        // Prepare mock error response
        val errorResponse = """{"message": "Invalid pulse rate"}"""

        with(mockConnection) {
            every { requestMethod = any() } just runs
            every { setRequestProperty(any(), any()) } just runs
            every { doOutput = any() } just runs
            every { outputStream } returns ByteArrayOutputStream()
            every { responseCode } returns HttpURLConnection.HTTP_BAD_REQUEST
            every { errorStream } returns ByteArrayInputStream(errorResponse.toByteArray())
            every { disconnect() } just runs
            every { sslSocketFactory = any() } just runs
            every { hostnameVerifier = any() } just runs
        }

        // Mock JSONObject for both request and error response
        mockkConstructor(JSONObject::class)
        // Mock for request JSONObject creation and modification
        every { anyConstructed<JSONObject>().put("average_pulse", 300) } returns mockk()
        every { anyConstructed<JSONObject>().put("duration", 30) } returns mockk()
        every { anyConstructed<JSONObject>().toString() } returns """{"average_pulse":300,"duration":30}"""
        // Mock for error response parsing
        every { anyConstructed<JSONObject>().getString("message") } returns "Invalid pulse rate"

        // Execute request
        val result = makeAddNewRunningSessionRequest(
            duration = 30,
            averagePulse = 300,
            password = null,
            context = mockContext,
            testConnection = mockConnection
        )

        // Verify error
        assertTrue(result is RunningSessionResult.Error)
        assertEquals("Invalid pulse rate", (result as RunningSessionResult.Error).message)
    }

    @Test
    fun `test unauthorized session addition`() = runBlocking {
        // Setup connection for unauthorized response
        with(mockConnection) {
            every { requestMethod = any() } just runs
            every { setRequestProperty(any(), any()) } just runs
            every { doOutput = any() } just runs
            every { outputStream } returns ByteArrayOutputStream()
            every { responseCode } returns HttpURLConnection.HTTP_UNAUTHORIZED
            every { disconnect() } just runs
            every { sslSocketFactory = any() } just runs
            every { hostnameVerifier = any() } just runs
        }

        // Execute request
        val result = makeAddNewRunningSessionRequest(
            duration = 30,
            averagePulse = 140,
            password = null,
            context = mockContext,
            testConnection = mockConnection
        )

        // Verify error
        assertTrue(result is RunningSessionResult.Error)
        assertEquals("Session expired. Please login again", (result as RunningSessionResult.Error).message)
    }
}